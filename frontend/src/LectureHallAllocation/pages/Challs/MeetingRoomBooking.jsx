...
export default function MeetingRoomList() {
  ...
  const validateForm = () => {
    ...
    if (!newRoom.LID.trim()) {
      ...
      !editingRoom && // Only check uniqueness when creating a new room
      allRooms.some((room) => room.LID === newRoom.LID)
    ) {
      newErrors.LID = "Room ID already exists";
    } else if (
      editingRoom && // When editing, check uniqueness against other rooms
      allRooms.some((room) => room.LID === newRoom.LID && room._id !== editingRoom._id)
    ) {
      newErrors.LID = "Room ID is already taken by another room";
    }
    ...
    } else if (parseInt(newRoom.floor, 10) < 0) {
    ...
    } else if (isNaN(newRoom.totalSeats) || parseInt(newRoom.totalSeats, 10) <= 0) {
      newErrors.totalSeats = "Total Seats must be a positive number";
    } else if (!Number.isInteger(parseFloat(newRoom.totalSeats))) {
